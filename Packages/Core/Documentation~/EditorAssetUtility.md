...
- `CreateConfigAsset<T>` and `CreatePrefabVariant` both reject a null/empty `folderPath` or `assetName` up front with an `Error` log and return `null`, before any path is built or folder is touched.
