...
## Setup
1. `Assets/Create/BFTools/Core/Config/Logger Config` creates a config at `Assets/Resources/BFTools/BFLoggerConfig.asset`. Alternatively, running `BF Tools/New Project Setup` (in `com.bftools.editortools`) creates the same asset at the same path as one of its steps.
