...
- `Trigger` sets `Time.timeScale` directly to the entry's `timescale`, cancelling any in-progress restore coroutine first (hitstops don't stack).
- After `duration` seconds (`WaitForSecondsRealtime`, unaffected by the timescale change itself), `Time.timeScale` is restored to `1f`.
- Cancelling an active hitstop (a new trigger or the component disabling) restores `Time.timeScale` to `1f` immediately rather than leaving the game stuck at the frozen timescale.
