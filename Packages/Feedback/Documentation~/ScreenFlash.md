...
4\. Assign one or more `ScreenFlashConfig` assets to the variant's `BFScreenFlash` component (`configs` list) — e.g. split by category (`UI\_ScreenFlash`, `Combat\_ScreenFlash`, `Environment\_ScreenFlash`) instead of cramming everything into one asset.
...
\- Stops any in-progress flash coroutine first (flashes don't stack), then starts a new one.
