...
- Target is resolved as `Camera.main.transform` on `OnEnable` and re-resolved on every `SceneManager.sceneLoaded`. Any active shake is cancelled first, restoring the previous target's original position before the new target takes over.
- On event, looks up `eventName` in the merged lookup; no match, no configs, or no target means no-op.
- `Trigger` cancels any active shake coroutine before starting a new one (shakes don't stack).
...
- Cancelling an active shake (a new trigger, a scene load, or the component disabling) restores the target's original position immediately rather than leaving it offset mid-shake.
