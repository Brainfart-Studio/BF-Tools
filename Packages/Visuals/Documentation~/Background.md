...
- Three concrete layers ship today.
  ...
  - **Aurora Ribbons** renders a configurable number of animated, additively-glowing `LineRenderer` ribbons, each tinted by cycling through a `Ribbon Colors` list and faded overall via Overall Opacity. Ribbons are spaced out via Ribbon Spacing independent of Thickness, so they can be packed tight, overlapped, or spread wide, and the whole band can be rotated (Angle) and animated (Rotation Speed, Rotation Oscillation Speed/Amplitude). Each ribbon's thickness, amplitude, wave frequency, and speed are drawn from Min/Max variance ranges around their respective base values (Thickness, Amplitude, Wave Frequency, Wave Speed), plus a secondary, smaller wave layered on top (Secondary Wave Strength, Secondary Wave Speed Scale). Ribbons span the full screen diagonal so their ends stay hidden off-screen at any rotation angle instead of only at 0°. All of the above refresh live in Play mode as the config changes.
